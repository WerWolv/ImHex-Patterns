...
| Hotline Bookmark || [`patterns/hotline_Bookmark.hbm.hexpat`](patterns/hotline_Bookmark.hbm.hexpat) | Hotline Connect Bookmark file (*.hbm) |
| Hotline Resources || [`patterns/hotline_Resources.dat.hexpat`](patterns/hotline_Resources.dat.hexpat) | Hotline/AppWarrior "AWRZ" flat resource file (*.dat) |
| Hotline UserData || [`patterns/hotline_UserData.hexpat`](patterns/hotline_UserData.hexpat) | Hotline Server UserData file |
...
| WFM | | [`patterns/wfm.hexpat`](patterns/wfm.hexpat) | Tektronix waveform file |
...
Every name listed below is a file: `encodings/<name>.tbl`, lowercased and
otherwise unchanged, so `#pragma encoding <name>` works for any of them.
Encodings with identical entries share them with an `-include` line instead
of repeating them (every ASCII-transparent encoding includes `ascii`, for
example), and a file named after an alias holds a single `-alias` line.
