...
# Every file this script writes starts with this line.
# A file without it is left alone.
...
# codec -> (display name, description), both hand-curated. The name
# is a label, not the filename.
CODEC_ENCODINGS = {
...
# Encodings derived from another one's entries plus overrides.
# source: key to copy entries from. key_filter: keep only matching
# keys. overrides: entries to add or replace.
DERIVED_ENCODINGS = {
    "jis_x0201": {
        "name": "JIS X 0201",
...
def normalize(name):
    ...
    are left alone, so "iso8859-1" and "iso_8859_1" are two files."""
    return name.lower()
...
def discover_shared_bases(real_full):
    ...
    such table, and only if that beats its current base. A stem already
    used as a base, like ascii or gbk, never switches itself, though
    others can still share with it.
